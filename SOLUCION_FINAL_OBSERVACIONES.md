...
**?? Causa:**
- `UnitOfWork` no exponía la propiedad `Context` públicamente
- `UsuarioService` intentaba acceder al contexto para realizar hard delete
- Acceso directo al DbContext necesario para eliminar físicamente registros

**? Soluciones Aplicadas:**

1. **UnitOfWork.cs** - Expuesta propiedad Context:
```csharp
public class UnitOfWork : IUnitOfWork
{
    private readonly SciFiHubDbContext _context;

    // ? NUEVA: Propiedad pública para acceso al contexto
    public SciFiHubDbContext Context => _context;

    // ...resto del código
}
```

2. **UsuarioService.cs** - Corrección del método EliminarDefinitivamente:
```csharp
public async Task<Result> EliminarDefinitivamenteAsync(Guid usuarioId, ...)
{
    // ...validaciones

    // ? ANTES: (dynamic)_unitOfWork.Context (causaba error)
    // ? AHORA: _unitOfWork.Context (propiedad pública)
...
| `SciFiHub.Infrastructure/Repositories/UnitOfWork.cs` | Agregada propiedad `Context` pública | Permitir hard delete |
| `Services/UsuarioService.cs` | Corregido acceso a Context | Eliminar definitivamente sin errores |
...
- ?? Context expuesto en UnitOfWork
